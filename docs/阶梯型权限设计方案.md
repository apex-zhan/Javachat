...
> **版本**：v2.1  
> **最后更新**：2026-06-15  
> **参考实践**：[IMA知识库架构设计](../mallchat-ai/docs/IMA知识库的实践方案.md)  
...
> **架构的生命力在于演进**。优秀的权限架构并非一蹴而就,而是在持续变化的需求中,通过不断识别并优化其薄弱点,逐步演进而成的。

本方案遵循IMA知识库实践的核心原则:

1. **识别并隔离变化** - 权限模型与业务逻辑解耦,支持独立演进
2. **架构的价值在于务实** - 结合C端实际场景,不追求过度设计
3. **可扩展性优先** - 为未来权限体系的复杂性预留扩展点
4. **统一权限网关** - 所有权限检查必须经过统一入口,避免散落判断
...
11. [AI 模块权限专项](#十一ai-模块权限专项)
12. [生产级补强与落地 checklist](#十二生产级补强与落地-checklist)
13. [变更记录](#十三变更记录)
...
| 全员群 | 全员群内不存在群主/管理员，仅全局 CHAT_MANAGER/ADMIN 可 `@all`，仅全局 ADMIN 可踢人 |
...
MallChat 存在「全员群」场景（`HotFlagEnum.YES`），人数众多，群内不设群主/管理员概念：

| 权限 | 普通群 | 全员群 |
|------|--------|--------|
| `@all` | 群主/群管理员可发 | 仅全局 CHAT_MANAGER/ADMIN 可发 |
| 踢人 | 群主/群管理员可踢 | 仅全局 ADMIN 可踢 |
| 邀请成员 | 群主/群管理员可邀 | 禁止邀请（已存在逻辑） |

实现上 `GroupPermissionService` 应先判断 `isHotGroup(groupId)`，再进入普通群权限矩阵。
...
| `TextMsgHandler` @all 校验 | 普通群：`globalPermissionService.hasPermission(uid, MSG_AT_ALL)`；全员群：需额外判断 `!isHotGroup` |
| `RoomAppServiceImpl#hasPower` | `groupPermissionService.hasPermission(...)`，内部覆盖全员群分支 |
...
`RateLimitConfig` 按 `globalLevel` 差异化配额（以下数值为**示例**，上线前根据实际运营数据调整）：
...
> **模块依赖**：把 `GlobalPermissionService` 接口下沉到 `mallchat-common-starter`（`com.abin.mallchat.common.permission` 包），AI 模块通过 `mallchat-common-starter` 依赖，避免循环依赖。`mallchat-chat-server` 提供实现类。
...
| 事件 | 失效策略 |
|------|----------|
| 分配/撤销全局角色 | 清除 `user:roles:{uid}` 缓存，发送 `PermissionChangeEvent` |
| 修改群成员角色 | 清除群成员缓存 `groupMemberCache`，发送 `PermissionChangeEvent` |
| 用户重新登录 | 重新拉取 permissions 列表 |
| 拉黑用户 | 发送 `PermissionChangeEvent`，前端收到后刷新权限 |
| 修改文档可见性 | 清除相关查询结果缓存，发送 `PermissionChangeEvent` |
...
> ⚠️ **聊天域 9.1~9.3 已确认 ✅ | AI 域 9.4 已确认 ✅**
...
| 14 | AI 权限包放哪？ | A. 抽到 `mallchat-common-starter`（共享 starter） / B. AI 模块自建依赖 chat-server | **A**（避免循环依赖） |
...
## 十二、生产级补强与落地 checklist

本章把 IMA 知识库「权限深度建模 + 统一权限网关」与 MallChat 实际相结合，给出第一阶段落地时必须补齐的生产级细节。

### 12.1 统一权限网关：与现有 URI 规范的衔接

项目已有约定（见《项目笔记.md》）：

> `/capi` 区分 C 端，`/public` 代表公开接口，`/admin` 代表需后台权限，其余需登录态。

阶梯型权限方案应与之衔接，形成两层网关：

| 层级 | 职责 | 现有实现 | 新增/改造 |
|------|------|----------|-----------|
| **URI 网关** | 判断接口是否公开/需登录 | `TokenInterceptor`、`BlackInterceptor` | 保持 |
| **权限网关** | 判断登录用户是否拥有某操作权限 | `IRoleService#hasPower`（旧） | `GlobalPermissionService`、`GroupPermissionService`、`AIPermissionService` |

原则：**URI 决定谁能进来，权限网关决定进来的人能做什么**。新方案不破坏 `/capi/*` 的既有规范。

### 12.2 权限变更的实时生效机制

拉黑、分配/撤销角色、修改群成员角色后，必须让缓存和前端立即感知。MallChat 已有事件驱动基础（MQ + Spring Event），建议统一收口：

```
权限变更事件（PermissionChangeEvent）
    ├── 清除 user:roles:{uid} 缓存
    ├── 清除 groupMember:{roomId} 缓存
    ├── 下发 WS 权限刷新指令（type = PERMISSION_REFRESH）
    └── 记录操作日志（AuditorLog）
```
...
| 操作 | 事件源 |
|------|--------|
| 拉黑用户 | `UserController#black` 成功后 |
| 分配/撤销全局角色 | 用户角色表变更后 |
| 修改群成员角色 | `GroupMember` 变更后 |
| 修改文档可见性 | `KnowledgeDocument` 更新后 |
| 全局禁言/解禁 | 禁言配置变更后 |

**前端行为**：收到 `PERMISSION_REFRESH` 后重新拉取 `userInfo` 或刷新页面；或在请求时被 403 后触发重新登录态校验。

### 12.3 全员群（Hot Group）的特殊处理

MallChat 存在「全员群」概念（`HotFlagEnum.YES`），人数可能上万。当前 `RoomAppServiceImpl#addMember` 已拒绝向全员群邀请成员。权限方案需注意：

1. **@all 权限**：全员群中 `@all` 应仅限全局 CHAT_MANAGER/ADMIN，群主/群管理员的 `@all` 权限在全员群中应被覆盖为不可使用，避免全站刷屏。
2. **踢人权限**：全员群不存在普通群主概念，禁止群内成员互相踢人，仅全局 ADMIN 可处理。
3. **撤回消息**：全员群消息量大，超管/群聊管理员撤回他人消息需加频控，防止滥用。

实现上，`GroupPermissionService` 增加 `isHotGroup(groupId)` 分支：

```java
public boolean hasGroupPermission(Long groupId, Long operatorUid,
                                  Long targetUid, GroupPermissionEnum perm) {
    if (isHotGroup(groupId)) {
        return hasHotGroupPermission(operatorUid, perm);
    }
    return hasNormalGroupPermission(groupId, operatorUid, targetUid, perm);
}
```

### 12.4 审计日志（最小必要版）

生产环境必须记录以下敏感操作，便于事后追溯：

| 操作 | 记录字段 | 存储方式 |
|------|----------|----------|
| 拉黑用户 | 操作者 uid、目标 uid/ip、原因、时间 | MySQL 异步写入 |
| 踢出群成员 | 操作者 uid、群 id、目标 uid、时间 | MySQL 异步写入 |
| 撤回他人消息 | 操作者 uid、消息 id、房间 id、时间 | MySQL 异步写入 |
| 修改文档可见性 | 操作者 uid、文档 id、旧值、新值 | MySQL 异步写入 |
| 分配全局角色 | 操作者 uid、目标 uid、角色 id、时间 | MySQL 异步写入 |

> 第一阶段不必引入独立审计系统表，可先用 `operation_log` 通用日志表（如项目已有）或 MQ 异步落库。

### 12.5 缓存策略（精简版）

第一阶段不推荐额外维护 `user:perm:{uid}` 缓存，原因：

1. 当前 `UserCache#getRoleSet(uid)` 已缓存用户角色集合，TTL 合理。
2. `permissions` 列表由角色静态推导，计算成本 O(角色数) ≈ O(1)。
3. 新增一层缓存会引入一致性问题，收益有限。

**推荐策略**：

```
GlobalPermissionService
    └── 直接读 UserCache#getRoleSet(uid)
        └── JetCache 自动管理角色集合缓存
```

当演进为第二阶段动态 RBAC 时，再引入 `user:perm:{uid}` 缓存。

### 12.6 安全红线

1. **禁止直接判断角色 ID**：所有业务代码必须通过 `*PermissionService` 判断权限点。遗留 `iRoleService.hasPower(uid, RoleEnum.ADMIN)` 需逐步迁移。
2. **前后端必须分别校验**：前端控制按钮显隐，后端接口必须独立校验，不能信任前端传参。
3. **AI 模块禁止信任请求体 userId**：必须以 `TokenInterceptor` 解析的 `RequestHolder.uid` 为准。
4. **敏感操作加频控**：拉黑、踢人、撤回他人消息、修改文档可见性建议加接口级频控（复用 `mallchat-frequency-control`）。
5. **缓存 key 隔离**：AI 查询结果缓存 key 必须包含 uid，防止跨用户命中。

### 12.7 限流数值说明

第 5.8.5 节的限流配额（L0 10 次/分钟等）为**示例值**，正式上线前需根据：

- 用户实际使用数据
- 模型供应商配额
- 成本预算

通过配置中心动态调整，不要硬编码在代码中。

### 12.8 测试要点

| 测试维度 | 必测场景 |
|----------|----------|
| 继承性 | ADMIN 拥有 CHAT_MANAGER 全部权限；群主拥有群管理员全部权限 |
| 域隔离 | 群主不自动获得全局 `@all` 权限；全局 CHAT_MANAGER 不自动成为某群群主 |
| 全员群 | 全员群中普通群主/群管理不能 `@all`，不能踢人 |
| AI 检索 | 普通用户只能查自己 PRIVATE 文档；未指定 documentId 时非 ADMIN 返回空/403 |
| 伪造 userId | 直接传请求体 userId 不应绕过权限 |
| 缓存失效 | 角色变更后 1 分钟内，旧权限不应生效 |
| 前端一致性 | 拉黑按钮对 CHAT_MANAGER 不可见，对 ADMIN 可见 |

### 12.9 落地 checklist

#### Phase 1 聊天域

- [ ] 新增 `GlobalPermissionEnum`、`GroupPermissionEnum`
- [ ] 新增 `GlobalPermissionService`、`GroupPermissionService`
- [ ] `IRoleService#hasPower` 标记 `@Deprecated`，内部委托 `GlobalPermissionService`
- [ ] 改造拉黑接口：`USER_BLACK` 仅 L2
- [ ] 改造撤回接口：`MSG_RECALL_ANY` 为 L1，`MSG_RECALL_SELF` 为 L0（2 分钟内）
- [ ] 改造 `@all`：L1 且非全员群；全员群仅 L2
- [ ] 改造踢人：集中进 `GroupPermissionService`，全员群仅 L2
- [ ] `WSLoginSuccess` 双写 `power` + 新增 `permissions`、`globalLevel`
- [ ] 前端修复拉黑按钮误显示
- [ ] 关键操作（拉黑、踢人、撤回他人消息）加审计日志

#### Phase 1B AI 域

- [ ] `ai_knowledge_document` 新增 `visibility` 字段（DEFAULT PRIVATE）
- [ ] AI 模块接入 `TokenInterceptor`，`RateLimitInterceptor` 从 `RequestHolder` 取 uid
- [ ] 新增 `AIPermissionEnum`、`AIPermissionService`
- [ ] RAG 检索增加文档归属校验；全局检索仅限 L2
- [ ] 文档更新/删除增加 `uploadUserId` 或 L2 校验
- [ ] 助手历史/会话接口绑定 Token uid
- [ ] 限流配置按 `globalLevel` 差异化
- [ ] AI 查询结果缓存 key 包含 uid
- [ ] AI 模块权限包落地位置：优先放到 `mallchat-common-starter` 下 `com.abin.mallchat.common.permission` 包
...
| v1.2 | 2026-05-30 | 新增缓存设计、性能优化、安全考虑、测试计划章节 |
| v2.1 | 2026-06-15 | 合并 12-15 章为「生产级补强与落地 checklist」；补充权限变更事件、全员群处理、审计日志、URI 网关衔接、缓存精简策略、AI 域依赖落地位置；修正测试/安全示例与项目命名不一致问题 |
